...
- **headless 会话 `Dispatch` 的 `Action` 重载不泵异步续体**（async lambda 即 async void）：服务内部 `await HttpClient` 之类的真异步调用挂进去会永久卡死。**`Dispatch(Func<Task>)` 在 12.1.2 同样不可信**（2026-09 实测：它在 lambda 首个 await 处即提前返回，既不等 lambda 完成也不传播其异常——**lambda 内的断言失败一律被静默吞掉**，`SettingsHeadlessTests` 曾因此整文件假绿，探针实验实锤）：正确模式是 **lambda 内只做无 await 的同步界面交互（建窗口/发事件/点击/读值存局部变量），断言与轮询一律放 Dispatch 之外**（先例：`SettingsHeadlessTests`）；文件级落盘断言用 `JsonSerializer` 反序列化后断模型值，不要对原始文件文本做 `Replace('\\','/')` + Contains（JSON 转义会把 `\\` 变 `//` 骗过它，同文件实锤）；非 UI 的异步服务调用在会话启动（`HeadlessSession.Instance` 触发全局 locator 初始化）后**测试线程直调**即可（先例：`BackgroundResilienceTests`）；要碰 UI 对象才进 Dispatch。
