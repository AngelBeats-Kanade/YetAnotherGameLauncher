...
4. **语义变更必须对照上一版**。改缓存/契约/顺序类语义时 `git show <prev> -- <file>` 并排对照：①保留的注释是否仍讲真话（M3 实锤：负缓存行为已改、旧注释原样保留，矛盾在提交里躺了两轮）；②旧纪律是否被静默丢弃；③对旧结构机制的断言**先验证再写**进注释/测试注释（M3 红值预估 12 实测 1——对旧代码的机制模型本身是错的）。
...
7. **无人值守轮的产出不 push**，收尾在轮记录里写明"待用户/下一轮 review 的面"。无人值守的闭环里没有第二视角，标注待审面是最低要求（§3.6 规则 1 的真机验证记录同样是它的一部分）。
