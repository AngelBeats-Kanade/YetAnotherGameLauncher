...
- 视觉自检：三个导出方法合计导出 16 张截图（01-15 + 02b，`Export_UiScreenshots_ForReview` 11 张 + `Export_LaunchErrorOverlay_ForReview` 4 张 + `Export_ProtonUpdateConfirm_ForReview` 1 张；2026-09-20 实测归属更正）到 `artifacts/ui-review/`（已 gitignore），改动 UI 后重跑并人工/judge 审查（`Export_LaunchErrorOverlay_ForReview` 覆盖启动失败覆盖层与 Linux 启动设置卡：umu 启动 + Proton 发行版下拉 + 检查更新按钮；`Export_ProtonUpdateConfirm_ForReview` 为 Proton 更新确认覆盖层，内含纱罩压暗的像素级回归断言）。
...
- **共享背景视频播放器（DI 单例）切游戏必须"停止即清帧"**：`Stop` 契约 = 取消循环 + 清空帧缓冲（`FfmpegVideoBackdropPlayer.StopCore`），配合 `PresentFrame` 代际门与 `GameItemViewModel` 的 `Frame 非空才点亮`——缺任何一层，上一游戏最后一帧的迟到通知（解码线程经 UI Dispatcher 异步投递）会让新详情页短暂显示旧画面（详见解法：ARCHITECTURE.md §3.7"停止即清帧"；回归：`VideoSource_SwitchingGames_LateStaleNotifyDoesNotLightNewGame`）。**切页是三态不是两态（2026-09-21）**：切另一游戏页 = 全停；切非游戏页 = `SuspendVideo` 暂停保活（`Pause` 泊车解码线程、帧/解码源/订阅保留，`HasBackgroundVideo` 不翻）；重进同一游戏页 = 续播快路径（会话存活直接 `Resume`，不起播不延迟）。`_activeVideoPage` 在切非游戏页后**仍指向被暂停的游戏页**（直到被另一游戏页替换）——从非游戏页再切游戏时才能正确驱动旧页全停换源，置 null 会让暂停会话悬挂订阅、误把别的游戏会话当自己的续播。
