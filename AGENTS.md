...
- **BoxShadow 无头渲染正常、真机渲染管线（原生 Wayland + NVIDIA + HDR 输出）会呈成边缘生硬的灰色矩形板**（2026-09-17 实锤：toast 卡阴影在用户屏幕上是"卡片同尺寸的硬边灰板"，观感即用户报告的"四角不是圆角、有矩形背景层"；同刻 grim 抓帧里阴影却几乎不存在——合成器截图缓冲与 HDR 扫描输出两条路径都不对，headless 截图则完全正常，三路互证）。弹层类 UI（toast/错误卡/修复确认条）的层次改用「描边 + 近实心底」表达，不要用 BoxShadow（NavIndicator 的 5px 小辉光保留为已知例外）。
