...
- 视觉自检：`UiScreenshotTests.Export_UiScreenshots_ForReview` 导出 15 张截图（2026-09-14 实测）到 `artifacts/ui-review/`（已 gitignore），改动 UI 后重跑并人工/judge 审查（`Export_LaunchErrorOverlay_ForReview` 导出启动失败覆盖层与 Linux 启动设置卡：umu 启动 + Proton 发行版下拉 + 检查更新按钮；`Export_ProtonUpdateConfirm_ForReview` 导出 Proton 更新确认覆盖层，内含纱罩压暗的像素级回归断言）。
