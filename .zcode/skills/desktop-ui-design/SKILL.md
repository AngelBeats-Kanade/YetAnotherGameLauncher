...
- 执行规则（2026-09-23 刻度审计固化）：**≥8 的间距必须取 4 的倍数，<8 的微间距必须取偶数**；
  唯一豁免：内容卡顶部 `0,46` 与标题色带 46 的结构对齐。

## 2. 字体层级（四组九档，2026-09-23 刻度审计定稿）

| 组 | 档位 | 用途 |
|---|---|---|
| Display | 24 / 28 SemiBold·Bold | 页面/游戏大标题，每屏最多一个 |
| Title | 18 / 20 SemiBold | 次级页面标题、区块标题 |
| Body | 13 / 14 / 15 / 16 | 正文、列表项、按钮 |
| Caption | 12 / 13（弱化色） | 辅助说明、版本号、标签（`AppTextSecondary` 系弱化） |

- 只用九档：12 / 13 / 14 / 15 / 16 / 18 / 20 / 24 / 28，禁止 .5 步进与档外值。
