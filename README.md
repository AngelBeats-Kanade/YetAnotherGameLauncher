...
| 现代化 UI | 海报式布局（参考 anime-games-launcher / Lutris）：全窗口游戏背景大图 + 模糊 + 渐变遮罩，玻璃质感侧栏与卡片；亮/暗/跟随系统三态主题；可折叠侧栏；页面切换与按钮微动效 |
| 界面语言 | 简体中文 / English，跟随系统可选，切换即时生效（设置页调整） |
| 启动设置 | 游戏详情页内直接编辑命令模板 / 工作目录 / 环境变量并保存回配置文件 |
| 存储路径可视化配置 | 设置页直接修改安装根目录，保存后游戏路径即时重新解析（Windows / Linux 通用，支持 `~` 展开） |
...
官方服务器与背景图（`schemaVersion` 一次性写入，仅执行一次），状态栏会提示补了什么。
