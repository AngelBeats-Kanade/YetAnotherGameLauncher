...
> 状态：已批准（2026-09-05）。本文件为项目实施蓝图，随开发进度在文末勾选。

## 一、调研结论

**本机环境**：.NET 10 SDK 10.0.400（最新 LTS，支持至 2028）；目录中已有 Rider 生成的 Avalonia 模板项目（Avalonia 12.1.2 + CommunityToolkit.Mvvm 8.4.2）。开发在 Windows 进行，目标运行平台为 Linux（代码全跨平台，Linux 打包用 self-contained publish）。
...
**依赖版本（均已核实为最新稳定非预览版）**：Avalonia 12.1.2、CommunityToolkit.Mvvm 8.4.2、Microsoft.Extensions.* 10.0.11、xunit.v3 4.0.0（Avalonia.Headless.XUnit 12.1.2 已迁移至 xunit v3 体系）、xunit.runner.visualstudio 4.0.0、Microsoft.NET.Test.Sdk 18.9.0。
...
| 测试 | xunit.v3 4.0.0 + xunit.runner.visualstudio 4.0.0 + Microsoft.NET.Test.Sdk；UI 测试用 Avalonia.Headless.XUnit 12.1.2（Avalonia 官方 Headless 库） |
...
│   └── YetAnotherGameLauncher/                  # Avalonia UI（现有模板项目移入 src/）
...
├── tests/
│   ├── YetAnotherGameLauncher.Core.Tests / .Channels.Kuro.Tests / .Channels.Hypergryph.Tests
│   │                                            # xunit.v3 + 假 HttpMessageHandler + 录制 JSON fixtures
│   └── YetAnotherGameLauncher.App.Tests         # Avalonia.Headless.XUnit
  ...
  "games": [
    {
      ...
      "launch": {
        "commandTemplate": "{exe}",            // Linux 上可写 "wine {exe}" / "steam -applaunch ..."，不硬编码
...
FluentTheme 单窗口布局：左侧边栏为游戏列表（图标+名称，底部设置按钮），主区域为选中游戏详情页（横幅图、版本状态"本地 x.y.z / 最新 a.b.c / 预下载可用"徽标、启动/下载/更新/预下载/校验修复按钮、下载进度条+速度+暂停取消），顶栏主题三态切换（跟随系统/亮/暗）。配色走集中资源字典，亮暗各一套 brush 覆盖；卡片式布局、圆角、留白。中文界面，文案集中在资源字典便于后续本地化。
