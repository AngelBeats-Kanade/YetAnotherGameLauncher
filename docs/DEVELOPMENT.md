...
**行为迁移的红阶段从盘点存量开始**：改"某行为从 A 挪到 B"类问题前，先 `git grep` 测试里钉住旧行为（A）的断言并列入迁移清单，随红阶段一并迁移——不盘点就会在全量跑测时以"预料外失败"的事后形态撞见（2026-09-23 实锤：Save 槽路由迁移漏了 LaunchSettingsMatrixTests 两处钉）。
...
**复现测试要症状级**：对未改动的代码跑它，必须以用户报告的症状方式红（例：反馈消息渲染在错误的卡里），而不是只在新增的 API/属性上红（新槽为空）——后者是规格测试，锁的是修复方案而非缺陷本身，症状可能从未被真正复现（2026-09-23 实锤）。最稳做法：先写症状级复现确认红，再写规格测试锁定新行为。
...
4. AGENTS.md「文档同步」耦合表已执行（review 时逐行对照该表，每条"无漂移"结论必须附实际跑过的查证命令——无证据的"没问题"不算执行过，2026-09-23 实锤：断言 README 无漂移却漏了测试数节）；
5. 涉 UI 的改动跑过视觉自检（UiScreenshotTests 导出截图 + 审查）——截图必须**本次重导**后审查，artifacts/ 是 gitignored 持久目录，审查前先验 mtime，别拿旧图当新产物（2026-09-23 实锤）。
...
### 3.6 机器状态依赖路径与兜底移除纪律（2026-09-24 起，适用于所有会话与无人值守轮）

背景：2026-09-24 无人值守轮 5af2855 移除了 FFmpeg 系统库符号链接回退（意图正确——那是错版本混绑），但下载目录因「解压摊平 tar 符号链接」从未真正完好，回退恰是这台机器上视频能播的承重墙——移除后背景视频全灭，而全量 873 测试全绿（离线套件结构性覆盖不到"绑定成功"路径）。三条规则：

1. **改动无法离线验证成功路径的代码（原生库加载、真实环境探测、硬件解码…）时，DoD 必须含真机验证记录**：跑应用 + 查日志/进程状态（如 `/proc/<pid>/maps` 库映射、`FFmpeg libraries ready` 日志），全量套件绿在此不构成证据。测试夹具到不了的路径，在测试类的 `<summary>` 里显式声明盲区与真机验证方式——盲区不声明 = 下一个人默认它被覆盖了。
...
同日五轮复盘沉淀的延伸规则——守卫判别力自查、机器前提显式 Skip、修复批次假设审计、语义变更对照上一版、环境变体实验、机械操作语义抽检、无人值守轮只扫不修——见 AGENTS.md「复审与修复纪律」节（该节为权威定义，此处不复制）。
