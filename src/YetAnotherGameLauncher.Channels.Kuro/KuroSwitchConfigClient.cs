...
{
    /// <summary>背景配置对时效不敏感：3 秒超时，拿不到就走本地缓存扫描兜底，不阻塞启动链路。</summary>
    ...
    /// <summary>直连拉取并解析配置；无背景投放（官方配置无 backgroundFile）或网络失败返回 null。</summary>
    ...
    public async Task<KuroSwitchConfig?> FetchAsync(string? indexUrl, CancellationToken cancellationToken = default)
    {
        ...
        // _t 为缓存破坏时间戳，与官方启动器请求形态一致，保证拿到最新投放
