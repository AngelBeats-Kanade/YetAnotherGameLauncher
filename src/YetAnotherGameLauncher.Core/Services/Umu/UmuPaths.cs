...
/// 路径均可注入，便于测试；数据根缺省取 AppPaths.DataHomeDirectory（Linux 尊重 XDG_DATA_HOME，
/// 进程启动时冻结），缓存根缺省每调用实时读 XDG_CACHE_HOME。
/// </summary>
public static class UmuPaths
{
    ...
    public static string LocalRoot(string? dataHome = null) =>
        Path.Combine(dataHome ?? AppPaths.DataHomeDirectory, "umu");
    ...
    public static string SteamCompatRoot(string? dataHome = null) =>
        Path.Combine(dataHome ?? AppPaths.DataHomeDirectory, "Steam", "compatibilitytools.d");
