    ...
    public async Task<bool> PlayAsync(string videoPath, CancellationToken cancellationToken = default)
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            finally
            {
                ...
                // 抛 ObjectDisposedException（2026-09-26 探针仲裁，docs/PITFALLS.md §1）——
                // 切页/切游戏的 SetDetailActive→StopVideo 触达，StopCore 的 catch(ODE) 承重。
                ...
                // 自然结束路径同样先取消再释放：循环点分析后台任务持同一令牌顺序软解全片
                //（预卷任务受 PrerollFrames 上限快速收敛），不取消就白烧解码 CPU——
